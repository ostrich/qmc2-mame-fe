...
isEmpty(QTSCRIPT_BACKEND): QTSCRIPT_BACKEND = $$(QTSCRIPT_BACKEND)
isEmpty(QTSCRIPT_BACKEND): QTSCRIPT_BACKEND = quickjs
equals(QTSCRIPT_BACKEND, quickjs) {
    DEFINES += QCHDMAN_QTSCRIPT_BACKEND_QUICKJS
} else:equals(QTSCRIPT_BACKEND, jsc) {
    DEFINES += QCHDMAN_QTSCRIPT_BACKEND_JSC
} else {
    error("QTSCRIPT_BACKEND must be 'quickjs' or 'jsc'")
}
message("qchdman QtScript backend: $$QTSCRIPT_BACKEND")

isEmpty(QTSCRIPT_PREFIX) {
    equals(QTSCRIPT_BACKEND, quickjs): QTSCRIPT_PREFIX = $$(QTSCRIPT_QUICKJS_PREFIX)
    equals(QTSCRIPT_BACKEND, jsc): QTSCRIPT_PREFIX = $$(QTSCRIPT_JSC_PREFIX)
}
isEmpty(QTSCRIPT_PREFIX): QTSCRIPT_PREFIX = $$(QTSCRIPT_PREFIX)
!isEmpty(QTSCRIPT_PREFIX) {
    CONFIG -= link_prl
