...
bool QFtpDTP::parseDir(const QByteArray &buffer, const QString &userName, QUrlInfo *info)
{
    ...
    const QRegularExpressionMatch unixMatch = unixPattern.match(bufferStr);
    if (unixMatch.hasMatch() && unixMatch.capturedStart() == 0) {
    ...
    QRegularExpression dosPattern(QLatin1String("^(\\d\\d-\\d\\d-\\d\\d\\d?\\d?\\ \\ \\d\\d:\\d\\d[AP]M)\\s+"
                                     "(<DIR>|\\d+)\\s+(\\S.*)$"));
    const QRegularExpressionMatch dosMatch = dosPattern.match(bufferStr);
    if (dosMatch.hasMatch() && dosMatch.capturedStart() == 0) {
...
void QFtpPI::readyRead()
{
    ...
    while (commandSocket.canReadLine()) {
        ...
        QString endOfMultiLine(4, QLatin1Char(' '));
        endOfMultiLine[0] = QChar(u'0' + replyCode[0]);
        endOfMultiLine[1] = QChar(u'0' + replyCode[1]);
        endOfMultiLine[2] = QChar(u'0' + replyCode[2]);
...
//    qDebug("QFtpPI state: %d [processReply() intermediate]", state);
    ...
    if (rawCommand) {
        ...
        const QRegularExpressionMatch addrPortMatch = addrPortPattern.match(replyText);
        if (!addrPortMatch.hasMatch()) {
            ...
            QStringList lst = addrPortMatch.capturedTexts();
