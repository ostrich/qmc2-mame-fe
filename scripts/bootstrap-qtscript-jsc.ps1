...
param(
    ...
    [string] $Prefix = (Join-Path $PWD '.deps\qtscript-jsc'),
    [string] $WorkRoot = (Join-Path $PWD '.deps\qtscript-jsc-work'),
...
Write-Host "QtScript JSC $portRevision installed in $Prefix"
Write-Host 'Set QTSCRIPT_BACKEND=jsc when building qchdman with this prefix'
